-- Create a database names jwt_test;
-- CREATE DATABASE jwt_test;
-- from the cmdline do:
-- sudo psql -U postgres -d jwt_test -f <this_files_name>
...
-- -- -- then log in as: sudo -u token_master psql -d jwt_test
-- better than the alternative of:
-- sudo -u postgres psql
-- \c jwt_test;
-- set role token_master;
-- set search_path to jwt_test;

-- IF YOU NEED TO DROP EVERYTHING:
-- Drop database jwt_test;
...
begin;
alter database jwt_test owner to token_master;
